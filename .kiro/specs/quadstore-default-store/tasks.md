...
- [x] 1. Add QuadStore NuGet package references
  - [x] 1.1 Add `QuadStore.Core` and `QuadStore.SparqlServer` PackageReferences to `src/fifthlang.system/Fifth.System.csproj` alongside the existing `dotNetRdf` reference
  ...
  - [x] 1.2 Ensure QuadStore packages propagate as transitive dependencies via the Fifth SDK
...
- [x] 2. Implement `Store.CreateFileStore` factory method and new KG built-in functions
  - [x] 2.1 Add `Store.CreateFileStore(string path)` to `src/fifthlang.system/Store.cs`
  ...
  - [x] 2.2 Add `remote_store`, `local_store`, `mem_store` built-in functions to `src/fifthlang.system/KnowledgeGraphs.cs`
  ...
  - [x] 2.3 Deprecate `sparql_store` and update `CreateStore()`
  ...
  - [x] 2.4 Write unit tests for `Store.CreateFileStore` and new KG built-in functions
  ...
  - [x] 2.5 Write property test: Save/Load round-trip on QuadStore-backed stores (Property 1)
  ...
  - [x] 2.6 Write property test: Persistence round-trip across store instances (Property 2)
...
- [x] 3. Checkpoint - Ensure all runtime tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Generalize parser grammar for store declarations
  - [x] 4.1 Update `colon_store_decl` rule in `src/parser/grammar/FifthParser.g4`
  ...
  - [x] 4.2 Update `AstBuilderVisitor.VisitColon_store_decl` in `src/parser/AstBuilderVisitor.cs`
  ...
  - [x] 4.3 Emit deprecation warning diagnostic `STORE_DEPRECATED_001` for `sparql_store` usage
    ...
    - _Requirements: 9.2_
  - [x] 4.4 Write parser tests for new store declaration forms
  ...
  - [x] 4.5 Write property test: Parser produces correct AST for all store creation functions (Property 3)
...
- [x] 5. Checkpoint - Ensure parser and runtime tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 6. Update query execution for QuadStore-backed stores
  - [x] 6.1 Update `QueryApplicationExecutor.Execute` in `src/fifthlang.system/QueryApplicationExecutor.cs`
  ...
  - [x] 6.2 Write unit tests for query execution against QuadStore-backed stores
    ...
    - _Requirements: 7.1, 7.2, 7.3, 7.4_
  - [x] 6.3 Write property test: Query result type matches query form (Property 4)
...
- [x] 7. Verify graph interoperability and TriG literal support
  - [x] 7.1 Verify `Store.LoadFromTriG` continues to work and graph operators function with QuadStore-backed stores
  ...
  - [x] 7.2 Write property test: TriG literal loading produces functional store (Property 5)
  ...
  - [x] 7.3 Write integration tests for graph interoperability with QuadStore
...
- [x] 8. Update documentation
  - [x] 8.1 Update `docs/Getting-Started/knowledge-graphs.md` with new store creation functions
