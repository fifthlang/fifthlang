...
- [ ] 1. Create core pipeline types and interfaces
  - [ ] 1.1 Create `src/compiler/Pipeline/ICompilerPhase.cs` with the `ICompilerPhase` interface
    - Define `Name`, `DependsOn`, `ProvidedCapabilities` properties and `Transform(AstThing, PhaseContext)` method
    - _Requirements: 1.1, 1.2, 1.3, 1.4, 1.5_
  - [ ] 1.2 Create `src/compiler/Pipeline/PhaseResult.cs` record type
    - Define `TransformedAst`, `Diagnostics`, `Success` fields and `Ok`/`Fail` factory methods
    - _Requirements: 2.1, 2.2, 2.3, 2.4_
  - [ ] 1.3 Create `src/compiler/Pipeline/PhaseContext.cs` class
    - Define `Diagnostics`, `TargetFramework`, `SharedData`, `EnableCaching` members
    - _Requirements: 3.1, 3.2, 3.3, 3.4, 3.5_
  - [ ] 1.4 Create `src/compiler/Pipeline/PipelineOptions.cs` record type
    - Define `SkipPhases`, `StopAfter`, `StopOnError`, `EnableCaching`, `DumpAfter`, `DumpCallback` and static `Default` property
    - _Requirements: 5.1, 5.2, 5.3, 5.4, 5.5, 5.6_
  - [ ] 1.5 Create `src/compiler/Pipeline/PipelineResult.cs` record type
    - Define `TransformedAst`, `Diagnostics`, `Success`, `PhaseTimings` fields
    - _Requirements: 4.4, 8.5_

- [ ] 2. Implement TransformationPipeline orchestrator
  - [ ] 2.1 Create `src/compiler/Pipeline/TransformationPipeline.cs`
  ...
  - [ ] 2.2 Implement `Execute(AstThing ast, PipelineOptions? options, string? targetFramework)` method
...
- [ ] 3. Checkpoint — Ensure core types compile
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 4. Implement concrete phase classes (first batch: phases 1–11)
  - [ ] 4.1 Create `src/compiler/Pipeline/Phases/TreeLinkagePhase.cs`
  ...
  - [ ] 4.2 Create `src/compiler/Pipeline/Phases/BuiltinInjectorPhase.cs`
  ...
  - [ ] 4.3 Create `src/compiler/Pipeline/Phases/ClassCtorsPhase.cs` (compound)
  ...
  - [ ] 4.8 Create `src/compiler/Pipeline/Phases/DefiniteAssignmentPhase.cs`
    ...
    - _Requirements: 6.1, 6.2, 6.3_
  - [ ] 4.9 Create `src/compiler/Pipeline/Phases/BaseConstructorValidationPhase.cs`
    - Wrap `SemanticAnalysis.BaseConstructorValidator`, merge `baseValidator.Diagnostics` into context
    - DependsOn: `"ResolvedConstructors"`. Provides: none
    - _Requirements: 6.1, 6.2, 6.3_
  - [ ] 4.10 Create `src/compiler/Pipeline/Phases/TypeParameterResolutionPhase.cs`
    - Wrap `TypeParameterResolutionVisitor`
    - DependsOn: `"Symbols"`. Provides: `"TypeParameters"`
    - _Requirements: 6.1, 6.2, 6.3_
  - [ ] 4.11 Create `src/compiler/Pipeline/Phases/GenericTypeInferencePhase.cs`
    - Wrap `GenericTypeInferenceVisitor`
    - DependsOn: `"TypeParameters"`. Provides: `"GenericTypes"`
    - _Requirements: 6.1, 6.2, 6.3_


- [ ] 5. Implement concrete phase classes (second batch: phases 12–24)
  - [ ] 5.1 Create `src/compiler/Pipeline/Phases/PropertyToFieldPhase.cs`
  ...
  - [ ] 5.2 Create `src/compiler/Pipeline/Phases/DestructurePatternFlattenPhase.cs` (compound)
  ...
  - [ ] 5.3 Create `src/compiler/Pipeline/Phases/OverloadGatheringPhase.cs`
    - Wrap `OverloadGatheringVisitor`
    - DependsOn: `"Destructuring"`. Provides: `"OverloadGroups"`
    - _Requirements: 6.1, 6.2, 6.3_
  - [ ] 5.4 Create `src/compiler/Pipeline/Phases/GuardValidationPhase.cs`
    ...
    - _Requirements: 6.1, 6.2, 6.3_
  - [ ] 5.5 Create `src/compiler/Pipeline/Phases/OverloadTransformPhase.cs`
    - Wrap `OverloadTransformingVisitor`
    - DependsOn: `"GuardChecks"`. Provides: `"OverloadTransforms"`
    - _Requirements: 6.1, 6.2, 6.3_
  - [ ] 5.6 Create `src/compiler/Pipeline/Phases/DestructuringLoweringPhase.cs`
...
- [ ] 6. Implement concrete phase classes (third batch: phases 25–38)
  ...
  - [ ] 6.5 Create `src/compiler/Pipeline/Phases/ExternalCallValidationPhase.cs`
  ...
  - [ ] 6.6 Create `src/compiler/Pipeline/Phases/TryCatchFinallyValidationPhase.cs`
  ...
  - [ ] 6.7 Create `src/compiler/Pipeline/Phases/QueryApplicationTypeCheckPhase.cs`
    ...
    - _Requirements: 6.1, 6.2, 6.3_
  - [ ] 6.8 Create `src/compiler/Pipeline/Phases/QueryApplicationLoweringPhase.cs`
    - Wrap `QueryApplicationLoweringRewriter`
    - DependsOn: `"QueryTypes"`. Provides: `"QueryLowered"`
    - _Requirements: 6.1, 6.2, 6.3_
  - [ ] 6.9 Create `src/compiler/Pipeline/Phases/ListComprehensionValidationPhase.cs`
    - Wrap `SparqlComprehensionValidationVisitor(context.Diagnostics)`, return `PhaseResult.Fail` on errors
    - DependsOn: `"Types"`. Provides: none
    - _Requirements: 6.1, 6.2, 6.3_
  - [ ] 6.10 Create `src/compiler/Pipeline/Phases/ListComprehensionLoweringPhase.cs`
    - Wrap `ListComprehensionLoweringRewriter`
    - DependsOn: `"Types"`. Provides: `"ComprehensionLowered"`
    - _Requirements: 6.1, 6.2, 6.3_
  - [ ] 6.11 Create `src/compiler/Pipeline/Phases/LambdaValidationPhase.cs`
    - Wrap `LambdaValidationVisitor(context.Diagnostics)`, return `PhaseResult.Fail` on errors
    - DependsOn: `"Types"`. Provides: none
    - _Requirements: 6.1, 6.2, 6.3_
  - [ ] 6.12 Create `src/compiler/Pipeline/Phases/LambdaClosureConversionPhase.cs` (compound)
  ...
  - [ ] 6.13 Create `src/compiler/Pipeline/Phases/DefunctionalisationPhase.cs` (compound)
  ...
  - [ ] 6.14 Create `src/compiler/Pipeline/Phases/TailCallOptimizationPhase.cs`
    - Wrap `TailCallOptimizationRewriter` + re-link (currently disabled/commented out)
    - DependsOn: `"Defunctionalised"`. Provides: none
    - _Requirements: 6.1, 6.5_

- [ ] 7. Implement `CreateDefault()` factory method
  - [ ] 7.1 Add `CreateDefault()` to `TransformationPipeline`
...
- [ ] 8. Checkpoint — Ensure all phases compile and `CreateDefault()` succeeds
  - Ensure all tests pass, ask the user if questions arise.


- [ ] 9. Migrate all callers and delete legacy code
  - [ ] 9.1 Update `src/compiler/Compiler.cs` `TransformPhase` method
    ...
    - _Requirements: 12.1_
  - [ ] 9.2 Update `src/language-server/Parsing/ParsingService.cs`
    - Replace `FifthParserManager.ApplyLanguageAnalysisPhases` call with pipeline API
    - _Requirements: 12.2_
  - [ ] 9.3 Update `src/language-server/SymbolService.cs` `BuildWorkspaceIndex` method
    - Replace `FifthParserManager.ApplyLanguageAnalysisPhases` call with pipeline API
    - _Requirements: 12.3_
  - [ ] 9.4 Update `test/ast-tests/TestInfrastructure/ParseHarness.cs`
    - Replace `FifthParserManager.ApplyLanguageAnalysisPhases` call with pipeline API
    - Change `ParseOptions.Phase` from `AnalysisPhase` enum to `string?` phase name
    - _Requirements: 12.4_
  - [ ] 9.5 Update `test/TestInfrastructure/ParseHarness.cs`
    - Same migration as 9.4 for the shared test infrastructure copy
    - _Requirements: 12.4_
  - [ ] 9.6 Migrate all remaining direct test calls to `ApplyLanguageAnalysisPhases`
    - Search for all remaining references and update to pipeline API
    - _Requirements: 12.5_
  - [ ] 9.7 Delete legacy code from `src/compiler/ParserManager.cs`
    - Delete the `AnalysisPhase` enum
    - Delete the `ApplyLanguageAnalysisPhases` method
    - Delete the `ExecutePhase` helper method
    - _Requirements: 12.6, 12.7, 12.8_

- [ ] 10. Checkpoint — Full regression gate
  - Run `dotnet test fifthlang.sln` to verify all existing tests pass with the new pipeline.
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 11. Pipeline structure verification tests
  - [ ] 11.1 Add `test/ast-tests/Pipeline/TransformationPipelineTests.cs` with unit tests
  ...
  - [ ]* 11.2 Write property test: Default pipeline structural integrity (Property 1)
  ...
  - [ ]* 11.3 Write property test: Dependency validation rejects unsatisfied dependencies (Property 2)
...
- [ ] 12. Pipeline execution behavior tests
  - [ ]* 12.1 Write property test: Phase execution respects registration order (Property 3)
  ...
  - [ ]* 12.2 Write property test: SkipPhases excludes named phases (Property 4)
  ...
  - [ ]* 12.3 Write property test: StopAfter halts at named phase (Property 5)
  ...
  - [ ]* 12.4 Write property test: StopOnError halts on failure (Property 6)
  ...
  - [ ]* 12.5 Write property test: Diagnostics accumulation (Property 7)
  ...
  - [ ]* 12.6 Write property test: Options forwarding to PhaseContext (Property 8)
  ...
  - [ ]* 12.7 Write property test: PhaseTimings populated (Property 9)
  ...
  - [ ]* 12.8 Write property test: DumpAfter invokes callback (Property 11)
  ...
  - [ ]* 12.9 Write property test: Phases property exposes all (Property 12)
  ...
  - [ ]* 12.10 Write property test: GetCapabilitiesAfter cumulative (Property 13)
  ...
  - [ ]* 12.11 Write property test: Exception handling preserves context (Property 15)
...
- [ ] 13. Behavioral equivalence and regression tests
  - [ ]* 13.1 Write property test: Pipeline behavioral equivalence (Property 14)
...
- [ ] 14. Final checkpoint — Full regression and new tests pass
