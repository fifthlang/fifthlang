namespace Fifth.Parser.LangProcessingPhases
{
    ...
    {
        public TypeAnnotatorVisitor(AstScopeAnnotations ast)
            => Annotations = ast;

        public AstScopeAnnotations Annotations { get; set; }
        public IScope CurrentScope { get; set; }
        public IScope GlobalScope { get; set; }

        public override void EnterFifthProgram(FifthProgram ctx)
            => CurrentScope = GlobalScope = Annotations.CreateNewScope(ctx);
        ...
        private void EnterScope(IAstNode ctx)
            => CurrentScope = Annotations.CreateNewScope(ctx, CurrentScope);
